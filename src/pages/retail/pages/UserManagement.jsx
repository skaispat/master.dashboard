...
const UserManagement = () => {
  ...
  const handlePageSelection = (page) => {
    setFormData(prev => {
      const currentPages = prev.retail_access || [];
      if (currentPages.includes(page)) {
        return {
          ...prev,
          retail_access: currentPages.filter(p => p !== page)
        };
      } else {
        return {
          ...prev,
          retail_access: [...currentPages, page]
        };
      }
    });
  };
  ...
  const handleEdit = (user) => {
    ...
    setFormData({
      ...
      retail_access: user.retail_access ? user.retail_access.split(',').map(p => p.trim()) : []
    });
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const accessString = formData.retail_access.join(',');
      ...
      if (editingUser) {
        ...
        // Use the original username as reference for update
        const { error } = await supabase
          .from('users')
          .update(updateData)
          .eq('username', editingUser.username); // Use original name for where clause
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-4 mb-4 shadow-sm">
      ...
      <div className="mb-3">
        ...
        <div className="flex flex-wrap gap-1">
          {user.retail_access ? (
            user.retail_access.split(',').slice(0, 3).map((page, index) => (
              <span
                key={index}
  ...
  <tr key={`${user.full_name}-${user.username}`} className="hover:bg-gray-50">
    ...
    <td className="px-6 py-4">
      <div className="flex flex-wrap gap-1 max-w-xs">
        {user.retail_access ? (
          user.retail_access.split(',').map((page, index) => (
            <span
              key={index}
              className="inline-flex items-center px-2 py-1 rounded-md text-xs bg-blue-100 text-blue-800"
            >
        ...
        >
          <Trash2 className="w-4 h-4" />
            ...
            <div className="lg:hidden p-4 max-h-[calc(100vh-280px)] overflow-y-auto">
              {filteredUsers.map((user) => (
                <UserCard key={`${user.full_name}-${user.username}`} user={user} /> 
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto p-2 border border-gray-200 rounded-lg">
              {availablePages.map((page) => (
                <label key={page} className="flex items-center space-x-2 p-2 hover:bg-gray-50 rounded">
                  <input
                    type="checkbox"
                    checked={formData.retail_access.includes(page)}
